...
void readDataPlayer(Player (*pl)[100]) {
    ...
    while (fgets(line, sizeof(line), dataPlayer)) {
        sscanf(line, "%s[^#]#%s[^#]#%d#%s[^#]", (*pl)[i].name, (*pl)[i].rank, (*pl)[i].score, (*pl)[i].date);
        i++;
    }*/

    // while (!feof(dataPlayer)) {
    //     fscanf(dataPlayer, "%[^#]#%[^#]#%d#%[^#]", (*pl)[i].name, (*pl)[i].rank, (*pl)[i].score, (*pl)[i].date);
    //     fflush(stdin);
    //     i++;
    // }

    while (!feof(dataPlayer)) {
        hasil = fscanf(dataPlayer, "%[^#]#%[^#]#%d#%[^\n]\n", (*pl)[i].name, (*pl)[i].rank, &(*pl)[i].score, (*pl)[i].date);
        if (hasil == 4) {
